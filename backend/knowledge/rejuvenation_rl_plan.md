...
*Last updated: 2026-05-02 (v3, post-implementation sprint).*
...
- Direct overlap of `biased_gene_signature` against the actual muscle aging atlases:
  - Lai et al. 2024 P2-up DE genes (young 15–35 vs aged 65+, pseudobulk DESeq2)
  - Lacraz 2024 P2-up DE genes (independent atlas — use as cross-validation)
...
#### Step E — CMAP / L1000 query skill [2 hrs]
New skill: `backend/skills/cmap_query/SKILL.md`
- Register at clue.io (free academic) → get API key → store in `backend/.env` as `CMAP_API_KEY`
- POST to `https://api.clue.io/api/perts` with {up: [...], down: [...]} gene signature
- Parse: return top-20 perturbagen matches with similarity scores
- Filter for muscle/aging relevance in the narrative (miniAgent/DeepSeek interprets the hits)
- Note: CMAP reference is mostly cancer cell lines — the signal is a proxy, not ground truth
...
#### Step H — `rl_loop.py` [3–4 hrs]
New file: `rl_loop.py` (project root or `backend/scripts/rl_loop.py`)
```python
import asyncio, json, httpx

MINIAGENT_URL = "http://localhost:8002/api/chat"
BUFFER_PATH = "backend/knowledge/experience_buffer.jsonl"

async def propose(context): ...       # POST to novokine_identification skill
async def validate(candidate): ...    # POST to novokine_validation skill  
async def score(candidate): ...       # POST to COT_Rejuv_Pipeline skill
                                      # (internally calls OmniPath, CMAP, P1/P2 overlap)
def log_to_buffer(result): ...        # append to experience_buffer.jsonl
async def design(candidate): ...      # POST to novokine_design_handoff skill

async def run_iteration(i):
    candidate = await propose(context=MUSCLE_CONTEXT)
    if not await validate(candidate): return
    result = await score(candidate)
    log_to_buffer(result)             # ALWAYS log, before design decision
    if result["reward"] == "GOOD" and result["reward_confidence"] >= 0.15:
        await design(candidate)
    elif result["reward"] == "GOOD" and result["reward_confidence"] < 0.15:
        flag_for_human_review(candidate, result)
...
#### Step I — Superbio design wiring [2–3 hrs]
- Check if Superbio exposes a REST API for the Binder Design with Boltz-1 pipeline
- If REST API exists: add `backend/utils/superbio_client.py` — POST design manifest, poll for completion, download results
- If no REST API: `novokine_design_handoff` skill outputs the manifest in a standardized format that can be pasted into Superbio Co-Pilot (semi-automated)
- Store design outputs in `backend/knowledge/design_outputs/{candidate_id}/`
...
| D | P1/P2 phenotypic checkpoint (Fisher/Jaccard) | ⬜ Next | Low | **Haiku** | Yes |
| E | CMAP/L1000 query skill | ⬜ Blocked (needs clue.io API key) | Medium | **Sonnet** | Yes |
...
| H | `rl_loop.py` orchestration | ⬜ Next | Medium | **Sonnet** | Yes |
| I | Superbio design wiring | ⬜ Next | Medium | **Sonnet** | Yes (code); API access is human task |
...
| `backend/knowledge/rejuvenation_rl_plan.md` | ✅ exists | **This file** — master plan |
...
| `backend/knowledge/experience_buffer.jsonl` | ⬜ to create | RL loop experience log |
...
| `backend/utils/phenotype_checkpoint.py` | ⬜ to create | P1/P2 Fisher exact + Jaccard overlap utility (Step D) |
| `backend/skills/cmap_query/SKILL.md` | ⬜ to create | CMAP/L1000 query skill (Step E, needs clue.io API key) |
| `backend/utils/biomni_client.py` | ⬜ to create | Biomni API wrapper (fallback only) |
| `backend/utils/superbio_client.py` | ⬜ to create | Superbio API wrapper (to be created) |
| `backend/scripts/rl_loop.py` | ⬜ to create | RL loop orchestration script (Step H) |
