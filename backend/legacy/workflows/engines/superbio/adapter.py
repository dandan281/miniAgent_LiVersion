...
# Bindings that map to Superbio ``local_files`` uploads rather than config keys.
# Value must be a local file path.  The mapping is: binding_key -> file_slot_name.
_LOCAL_FILE_BINDINGS: dict[str, str] = {
    "boltz2_input_file": "input_msa_file",   # Boltz-2: .a3m (single-chain) or .yaml (multi-chain)
    "protenix_input_file": "structure_file",  # Protenix: AF3-style JSON
}
...
def submit(
    ...
    *,
    ...
    # Build the app-side config dict and resolve local file uploads.
    config: dict[str, Any] = {}
    resolved_local_files: dict[str, str] = dict(local_files or {})
    for key, value in parameter_bindings.items():
        if key in _DISPATCH_KEYS:
            continue
        if key == "aa_pairs":
            config[key] = _coerce_aa_pairs(value)
        elif key in _LOCAL_FILE_BINDINGS:
            # Route to local_files upload rather than config; value is a local path.
            slot = _LOCAL_FILE_BINDINGS[key]
            resolved_local_files[slot] = str(value)
        else:
            config[key] = value
    ...
    try:
        raw = client.post_job(
            ...
            # Skip Superbio's client-side validator: it KeyErrors on apps with
            # empty parameter_settings (e.g. Boltz-2) and only adds redundant
            # checks our preflight runners already perform.
            validate=False,
        )
